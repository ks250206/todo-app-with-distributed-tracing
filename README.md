...
- UI state: Zustand
- Authentication: HMAC-SHA-256 pepper + Argon2id、Access Token、Refresh Token
...
パスワードはHMAC-SHA-256（key=`PASSWORD_PEPPER`）で混ぜたうえでArgon2idによりハッシュ化します。パスワードは12文字以上必要です。既存の連結方式で保存したハッシュとは互換がないため、方式変更後は再登録が必要です。
