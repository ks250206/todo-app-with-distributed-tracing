...
- `observability-Caddyfile`: JaegerとPrometheusのadmin認可gateway
- `otel-collector-config.yaml`: traceとspanmetrics pipeline
...
- 公開CaddyがBackendでadmin権限を検証し、共有secretで内部gatewayへの直接アクセスを防ぐ。JaegerとPrometheusのUIポートをホストへ直接公開しない。
...
- JaegerとPrometheusへのリンクは`role === "admin"`の場合だけ表示する。Frontendで隠すだけでなく、gatewayの認可を必ず維持する。
...
- Collectorのfilelog offsetを永続化し、再起動によるCaddy logの重複収集を避ける。
...
- `just start`はFrontendをproduction buildし、BackendをCargo release profileでbuildして、Caddy、Jaeger、Prometheus、Collector、内部gatewayとともに起動する。
- Caddy変更後は両方の設定を検証する。
